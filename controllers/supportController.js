...
const shapeUser = (user = null) => user ? {
  _id: user._id,
  name: user.name,
  email: user.email,
  role: user.role
...
const shapeSupportRequest = (request = {}) => ({
  ...
  tenant: request?.tenantId && typeof request.tenantId === "object" ? {
...
exports.createSupportRequest = async (req, res) => {
  ...
  notificationManager.createNotification({
    ...
    actions: [{
      ...
      type: "link",
      action: "/admin/admin-requests"
    }],
    metadata: {
      tenantId: String(tenantId),
      tenantName: request.tenantId?.name || "",
      tenantSlug: request.tenantId?.slug || "",
      tenantKey: request.tenantId?.key || "",
      supportRequestId: String(request._id),
      category,
      subject: request.subject
    }
  }).catch(error => {
    logger.error("Failed to create super admin support-request notification:", error);
  });
  return sendSuccess(res, 201, "Support request sent to super admin successfully", shapeSupportRequest(request));
};
exports.getSupportRequests = async (req, res) => {
  ...
  const requests = await SupportRequest.find(query).populate("tenantId", "name slug key status").populate("createdBy", "name email role").populate("updatedBy", "name email role").populate("respondedBy", "name email role").sort({
    createdAt: -1
  }).lean();
  return sendSuccess(res, 200, null, requests.map(request => shapeSupportRequest(request)), {
...
exports.updateSupportRequestStatus = async (req, res) => {
  ...
  const hasResponseMessageField = Object.prototype.hasOwnProperty.call(req.body || {}, "responseMessage");
  const request = await SupportRequest.findById(req.params.id);
  if (!request) {
    return sendError(res, 404, "Support request not found");
  }
  if (isSupportRequestLocked(request.status)) {
    return sendError(res, 409, "Resolved support requests are locked and cannot be updated");
  }
  if (hasStatus) {
  ...
  return sendSuccess(res, 200, hasResponseMessageField ? "Support request updated successfully" : `Support request marked as ${String(status).replace(/_/g, " ")}`, shapeSupportRequest(request));
};
