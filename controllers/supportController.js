...
exports.createSupportRequest = async (req, res) => {
  ...
  if (!tenantId) {
    return sendError(
      res,
      400,
      "Tenant workspace is required to contact super admin",
    );
  }
  ...
  notificationManager
    ...
    .catch((error) => {
      logger.error(
        "Failed to create super admin support-request notification:",
        error,
      );
    });
  return sendSuccess(
...
exports.updateSupportRequestStatus = async (req, res) => {
  ...
  const hasStatus =
    typeof status === "string" && String(status).trim().length > 0;
