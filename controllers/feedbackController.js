...
exports.submitFeedback = async (req, res) => {
  try {
    ...
    logger.error(error);
    if (
      error.message.includes("Customer session not found") ||
...
exports.getCustomerDetailsForFeedback = async (req, res) => {
  try {
    ...
    const customerDetails =
      await feedbackManager.getCustomerDetailsForFeedback(sessionId);
    if (!customerDetails) {
...
exports.getFeedbackForActiveSession = async (req, res) => {
  try {
    const { sessionId } = req.params;
    const feedback =
      await feedbackManager.getFeedbackForActiveSession(sessionId);
    if (!feedback) {
      ...
      feedback = feedback.filter((entry) => {
        ...
        return (
          ...
          String(entry.comments || "")
            .toLowerCase()
            .includes(keyword) ||
          String(entry.order?.orderNumber || "")
            .toLowerCase()
            .includes(keyword) ||
          String(entry.table?.tableNumber || "")
            .toLowerCase()
            .includes(keyword) ||
...
exports.getStaffPerformance = async (req, res) => {
  try {
    ...
    const performance = await feedbackManager.getStaffPerformance(
      startDate,
      endDate,
    );
    res.status(200).json({
...
exports.getFeedbackDashboard = async (req, res) => {
  try {
    const [recentFeedback, statistics, nps, trendingTopics] = await Promise.all(
      [
        Feedback.find()
          .populate("customer", "name email phone")
          .populate("order", "orderNumber")
