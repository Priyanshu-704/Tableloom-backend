...
const getRelationsForInventory = inventoryItem => {
  const relatedMenuItems = Array.isArray(inventoryItem?.relatedMenuItems) ? inventoryItem.relatedMenuItems : [];
  if (relatedMenuItems.length > 0) {
    return relatedMenuItems.filter(relation => relation?.menuItem).map(relation => ({
...
const serializeInventory = item => {
  ...
  const relatedMenuItems = getRelationsForInventory(inventory).map(relation => ({
    menuItem: relation?.menuItem && typeof relation.menuItem === "object" ? {
...
const notifyInventoryRequirement = async (inventoryItem, previousStatus = null, actorId = null) => {
  ...
  await notificationManager.createNotification({
    ...
    message: nextStatus === "out_of_stock" ? `${inventoryItem.ingredientName} is out of stock and needs replenishment.` : `${inventoryItem.ingredientName} is below minimum stock level.`,
    type: "inventory_low",
...
const syncMenuAvailability = async (menuItemIds = [], updatedBy = null) => {
  const normalizedMenuItemIds = [...new Set(menuItemIds.map(menuItemId => String(menuItemId || "").trim()).filter(Boolean))];
  if (!normalizedMenuItemIds.length) {
    return;
  }
  await Promise.all(normalizedMenuItemIds.map(async menuItemId => {
    ...
    const linkedIngredients = inventoryItems.map(inventoryItem => {
      const relation = getRelationsForInventory(inventoryItem).find(entry => String(entry.menuItem) === String(menuItemId));
      if (!relation) {
        return null;
      }
      return {
    ...
    const shouldBeAvailable = linkedIngredients.length === 0 || linkedIngredients.every(ingredient => ingredient.isActive && ingredient.currentStock >= ingredient.quantityRequired);
...
const validateAndNormalizeRelatedMenuItems = async (relatedMenuItems = []) => {
  const normalizedRelations = (Array.isArray(relatedMenuItems) ? relatedMenuItems : []).filter(relation => relation?.menuItem).map(relation => ({
    menuItem: String(relation.menuItem).trim(),
    quantityRequired: Math.max(Number(relation.quantityRequired || 0), 0)
  }));
  const uniqueMenuItemIds = [...new Set(normalizedRelations.map(relation => relation.menuItem))];
...
exports.getInventoryItems = async (req, res) => {
  try {
    ...
    if (category) {
      ...
      inventoryQuery.$and = [...(Array.isArray(inventoryQuery.$and) ? inventoryQuery.$and : []), {
        $or: [{
    ...
    const [items, total, allItems] = await Promise.all([InventoryItem.find(inventoryQuery).select("ingredientName sku unit currentStock minimumStock reorderQuantity notes isActive relatedMenuItems menuItem").populate({
      path: "relatedMenuItems.menuItem",
      select: "name isAvailable"
    }).sort({
      updatedAt: -1
    }).skip(skip).limit(limitNum), InventoryItem.countDocuments(inventoryQuery), InventoryItem.find({}).select("currentStock minimumStock isActive").lean()]);
    const stats = allItems.reduce((accumulator, item) => {
...
exports.createInventoryItem = async (req, res) => {
  try {
    ...
    const normalizedRelations = await validateAndNormalizeRelatedMenuItems(relatedMenuItems);
    ...
    await syncMenuAvailability(normalizedRelations.map(relation => relation.menuItem), req.user._id);
    ...
    const duplicateMessage = error.code === 11000 ? "Ingredient inventory already exists for this ingredient or SKU" : error.message;
...
exports.updateInventoryItem = async (req, res) => {
  try {
    ...
    const previousMenuItemIds = getRelationsForInventory(inventoryItem).map(relation => relation.menuItem);
    ...
    if (relatedMenuItems !== undefined) {
      inventoryItem.relatedMenuItems = await validateAndNormalizeRelatedMenuItems(relatedMenuItems);
      inventoryItem.menuItem = undefined;
    }
    inventoryItem.lastAdjustedAt = new Date();
    inventoryItem.updatedBy = req.user._id;
    await inventoryItem.save();
    await syncMenuAvailability([...previousMenuItemIds, ...getRelationsForInventory(inventoryItem).map(relation => relation.menuItem)], req.user._id);
    ...
    res.status(error.code === 11000 ? 409 : 400).json({
      success: false,
      message: error.code === 11000 ? "Inventory ingredient or SKU must be unique" : "Failed to update ingredient inventory",
      error: error.message
    });
  }
};
exports.adjustInventoryStock = async (req, res) => {
  try {
    ...
    inventoryItem.currentStock = Math.max(Number(inventoryItem.currentStock || 0) + signedQuantity, 0);
    inventoryItem.lastAdjustedAt = new Date();
    inventoryItem.updatedBy = req.user._id;
    ...
    await inventoryItem.save();
    await syncMenuAvailability(getRelationsForInventory(inventoryItem).map(relation => relation.menuItem), req.user._id);
    await notifyInventoryRequirement(inventoryItem, previousStatus, req.user._id);
    const populated = await InventoryItem.findById(inventoryItem._id).populate({
      path: "relatedMenuItems.menuItem",
      select: "name isAvailable"
...
exports.bulkUploadInventory = async (req, res) => {
  ...
  try {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      const ingredientNameValue = String(readCsvField(row, ["ingredientName", "ingredient", "name"])).trim();
      const skuValue = String(readCsvField(row, ["sku", "stockCode"])).trim().toUpperCase();
      try {
        const ingredientName = ingredientNameValue;
        const sku = skuValue;
        const unit = normalizeInventoryUnit(readCsvField(row, ["unit"]));
        const currentStock = parseInventoryNumber(readCsvField(row, ["currentStock", "current_stock", "stock"]), "Current stock");
        const minimumStock = parseInventoryNumber(readCsvField(row, ["minimumStock", "minimum_stock", "minThreshold"]), "Minimum stock");
        const reorderQuantity = parseInventoryNumber(readCsvField(row, ["reorderQuantity", "reorder_quantity", "reorder"]), "Reorder quantity");
        ...
        const isActive = isActiveValue === "" ? true : !["false", "0", "no", "inactive"].includes(isActiveValue);
        if (!ingredientName) {
        ...
        if (existingItem) {
          ...
          getRelationsForInventory(existingItem).forEach(relation => affectedMenuItemIds.add(String(relation.menuItem)));
          stats.updated += 1;
          stats.results.push({
            line: lineNumber,
            ingredientName,
          ...
          getRelationsForInventory(createdItem).forEach(relation => affectedMenuItemIds.add(String(relation.menuItem)));
          stats.created += 1;
          stats.results.push({
            line: lineNumber,
            ingredientName,
