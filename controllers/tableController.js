...
const sanitizeTable = (table, {
  ...
  const tableObj = table.toObject ? table.toObject() : {
    ...table
  };
  const sanitizedTable = {
    ...
    qrCode: tableObj.qrCode ? buildTenantAssetUrl(null, `/images/table-qr/${tableObj._id}`, tenant) : null,
  ...
  if (tableObj.qrTokenExpiry) {
    ...
    sanitizedTable.tokenDaysRemaining = Math.max(0, Math.ceil(remainingMs / (1000 * 60 * 60 * 24)));
...
exports.getTables = async (req, res) => {
  try {
    ...
    const tables = await Table.find(query).select("tableNumber tableName capacity location status qrCode qrToken qrTokenExpiry qrImageBucket qrPublicId qrProvider lastOccupied notes isActive").sort({
      tableNumber: 1
    }).skip(skip).limit(limitNum).lean();
    const sanitizedTables = tables.map(table => sanitizeTable(table, {
...
exports.getTable = async (req, res) => {
  try {
    const table = await Table.findById(req.params.id).select("tableNumber tableName capacity location status qrCode qrToken qrTokenExpiry qrImageBucket qrPublicId qrProvider lastOccupied notes isActive").lean();
    if (!table) {
...
exports.updateTableStatus = async (req, res) => {
  try {
    ...
    if (table.status === "occupied" && (status === "available" || status === "cleaning")) {
...
exports.deleteTable = async (req, res) => {
  try {
    ...
    if (activeSession) {
      return res.status(400).json({
        success: false,
        message: "Cannot delete table with active session. Please end the session first.",
        sessionId: activeSession.sessionId
      });
    }
    if (table.qrCode) {
...
exports.getTableStatistics = async (req, res) => {
  try {
    const [totalTables, availableTables, occupiedTables, reservedTables, maintenanceTables] = await Promise.all([Table.countDocuments({
      isActive: true
...
exports.downloadQRCode = async (req, res) => {
  try {
    ...
    res.setHeader("Content-Disposition", `attachment; filename="table-${table.tableNumber}-qrcode.png"`);
...
exports.refreshQRToken = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      data: {
        qrCode: table.qrCode ? buildTenantAssetUrl(null, `/images/table-qr/${table._id}`, req.tenant) : null,
        tokenExpiry: qrInfo.expiry,
        tokenDaysRemaining: 30,
        qrUrl: qrInfo.url,
        tokenExpired: false
...
exports.getQRTokenStatus = async (req, res) => {
  try {
    const table = await Table.findById(req.params.id).select("qrToken qrTokenExpiry tableNumber status isActive");
    if (!table) {
      return res.status(404).json({
        success: false,
        message: "Table not found"
    ...
    if (table.qrToken && table.qrTokenExpiry) {
      ...
      tokenStatus.daysRemaining = Math.ceil((new Date(table.qrTokenExpiry) - new Date()) / (1000 * 60 * 60 * 24));
    }
    res.status(200).json({
      success: true,
      data: tokenStatus
