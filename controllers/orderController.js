...
const shapeOrderResponse = (order = {}) => ({
  ...
  items: Array.isArray(order.items) ? order.items.map((item, index) => shapeOrderItemResponse(item, index)) : []
});
...
exports.getOrder = async (req, res) => {
  try {
    const order = await Order.findById(req.params.id).select("orderNumber status paymentStatus totalAmount currency specialInstructions preparationTime estimatedReadyTime orderPlacedAt createdAt table items").populate("table", "tableNumber tableName").populate("items.menuItem", "name").populate("items.sizeId", "name").lean();
    if (!order) {
...
exports.getOrderBySession = async (req, res) => {
  try {
    ...
    }).select("orderNumber status paymentStatus totalAmount currency specialInstructions preparationTime estimatedReadyTime orderPlacedAt createdAt table items").populate("table", "tableNumber tableName").populate("items.menuItem", "name").populate("items.sizeId", "name").sort({
      createdAt: -1
    }).lean();
    if (!order) {
      return res.status(404).json({
...
exports.getOrderHistoryBySession = async (req, res) => {
  try {
    ...
    let ordersQuery = Order.find({
      customer: customer._id
    }).select(summaryOnly ? "_id orderNumber status paymentStatus totalAmount orderPlacedAt createdAt items" : undefined).populate("items.menuItem", summaryOnly ? "name" : "name description image thumbnail category tags nutritionalInfo allergens ingredients").populate("items.sizeId", summaryOnly ? "name" : "name code").sort({
      orderPlacedAt: -1,
      createdAt: -1
    }).skip(skip).limit(limitNum).lean();
    if (!summaryOnly) {
      ordersQuery = ordersQuery.populate("customer", "sessionId name phone email").populate("table", "tableNumber tableName capacity location");
    }
    const [orders, total] = await Promise.all([ordersQuery, Order.countDocuments({
      customer: customer._id
    })]);
    const transformedOrders = summaryOnly ? orders.map(shapeOrderHistorySummary) : orders.map(order => transformOrderData(order));
...
exports.addItemsToOrder = async (req, res) => {
  try {
    ...
    logger.error(error);
    if (error.message.includes('Order not found') || error.message.includes('Cannot add items to order') || error.message.includes('Menu item not available') || error.message.includes('Selected size not available')) {
      return res.status(400).json({
        success: false,
...
exports.updateItemQuantity = async (req, res) => {
  try {
    ...
    const order = await orderManager.updateOrderItemQuantity(req.params.orderId, req.params.itemId, parseInt(quantity));
    res.status(200).json({
      success: true,
    ...
    logger.error(error);
    if (error.message.includes('Order not found') || error.message.includes('Order item not found') || error.message.includes('Cannot modify order')) {
      return res.status(400).json({
        success: false,
        message: error.message
      });
...
exports.removeItemFromOrder = async (req, res) => {
  try {
    const order = await orderManager.removeItemFromOrder(req.params.orderId, req.params.itemId);
    res.status(200).json({
      success: true,
    ...
    logger.error(error);
    if (error.message.includes('Order not found') || error.message.includes('Cannot modify order')) {
      return res.status(400).json({
        success: false,
        message: error.message
      });
...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    const order = await orderManager.updateOrderStatus(req.params.id, status, req.user.id, notes);
    res.status(200).json({
      success: true,
    ...
    logger.error(error);
    if (error.message.includes('Order not found') || error.message.includes('Invalid status transition')) {
      return res.status(400).json({
        success: false,
        message: error.message
      });
...
exports.processPayment = async (req, res) => {
  try {
    ...
    logger.error(error);
    if (error.message.includes('Order not found') || error.message.includes('Order is already paid')) {
      return res.status(400).json({
        success: false,
        message: error.message
      });
...
exports.getAllOrders = async (req, res) => {
  try {
    ...
    const skip = (pageNum - 1) * limitNum;
    let ordersQuery = Order.find(query).select("orderNumber status paymentStatus totalAmount currency specialInstructions estimatedReadyTime orderPlacedAt createdAt table items").populate("table", "tableNumber tableName").populate("items.menuItem", "name").populate("items.sizeId", "name").sort({
      orderPlacedAt: -1
    });
    if (search) {
    ...
    if (search) {
      ...
      transformedOrders = transformedOrders.filter(order => {
        const itemMatch = (order.items || []).some(item => String(item.menuItem?.name || item.name || "").toLowerCase().includes(keyword));
        return String(order.orderNumber || "").toLowerCase().includes(keyword) || String(order.table?.tableNumber || order.table?.tableName || "").toLowerCase().includes(keyword) || String(order.customer?.name || order.customer?.phone || "").toLowerCase().includes(keyword) || itemMatch;
      });
    }
    const total = search ? transformedOrders.length : await Order.countDocuments(query);
    const paginatedOrders = (search ? transformedOrders.slice(skip, skip + limitNum) : transformedOrders).map(({
      customer,
      ...order
    }) => order);
...
exports.getOrderStatistics = async (req, res) => {
  try {
    ...
    const [totalOrders, pendingOrders, preparingOrders, todayOrders, todayRevenue, popularItems, groupedStatusCounts] = await Promise.all([Order.countDocuments({
      orderPlacedAt
