...
const { generateAnalyticsReport, getGeneratedReport } = require("../utils/reportGenerator");
...
const buildKitchenAnalyticsDataset = async (rangeStart, rangeEnd) => {
  ...
  return {
    ...
    overallStats: {
      ...
      avgPreparationTime:
        analytics.length > 0
          ? analytics.reduce((sum, day) => sum + (day.avgPreparationTime || 0), 0) / analytics.length
          : 0,
      avgTotalTime:
        analytics.length > 0
          ? analytics.reduce((sum, day) => sum + (day.avgTotalTime || 0), 0) / analytics.length
          : 0,
      overallOnTimeRate:
        analytics.length > 0
          ? analytics.reduce((sum, day) => sum + (day.onTimeRate || 0), 0) / analytics.length
          : 0,
    },
    dateRange: {
      startDate: rangeStart,
...
const buildMenuDataset = async () => {
  const [totalItems, availableItems, unavailableItems, vegetarian, nonVegetarian, vegan, glutenFree] = await Promise.all([
    MenuItem.countDocuments({ isActive: true }),
    MenuItem.countDocuments({ isActive: true, isAvailable: true }),
    MenuItem.countDocuments({ isActive: true, isAvailable: false }),
    MenuItem.countDocuments({ isActive: true, isVegetarian: true }),
    MenuItem.countDocuments({ isActive: true, isNonVegetarian: true }),
    MenuItem.countDocuments({ isActive: true, isNonVegetarian: false, isVegetarian: false }),
...
const buildFinanceDataset = async (rangeStart, rangeEnd) => {
  ...
  const [summary, paymentMethods, dailyRevenue, orderTypes, sessionRevenue] = await Promise.all([
    Order.aggregate([
      {
        $match: {
          orderPlacedAt,
    ...
    Customer.aggregate([
      ...
      },
      {
        $group: {
          ...
          totalSessionRevenue: { $sum: { $ifNull: ["$totalSpent", 0] } },
          averageSessionRevenue: { $avg: { $ifNull: ["$totalSpent", 0] } },
        },
      },
    ]),
  ]);
...
const buildReportDataset = async (reportType, rangeStart, rangeEnd, tenantId) => {
  ...
  return getOrSetCache(cacheKey, REPORT_DATASET_CACHE_TTL_MS, async () => {
    ...
    const [orders, sessions, kitchen, feedback, menu, tables, waiterCalls] = await Promise.all([
