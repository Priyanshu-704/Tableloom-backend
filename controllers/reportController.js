...
const normalizeReportType = (value = "analytics") => {
  const normalized = String(value || "analytics").trim().toLowerCase();
...
const buildOrderStatisticsDataset = async (rangeStart, rangeEnd) => {
  ...
  const [totalOrders, pendingOrders, preparingOrders, paidRevenue, popularItems, groupedStatusCounts] = await Promise.all([Order.countDocuments({
    orderPlacedAt
...
const buildKitchenAnalyticsDataset = async (rangeStart, rangeEnd) => {
  ...
  return {
    ...
    overallStats: {
      totalOrders: analytics.reduce((sum, day) => sum + (day.totalOrders || 0), 0),
      avgPreparationTime: analytics.length > 0 ? analytics.reduce((sum, day) => sum + (day.avgPreparationTime || 0), 0) / analytics.length : 0,
      avgTotalTime: analytics.length > 0 ? analytics.reduce((sum, day) => sum + (day.avgTotalTime || 0), 0) / analytics.length : 0,
      overallOnTimeRate: analytics.length > 0 ? analytics.reduce((sum, day) => sum + (day.onTimeRate || 0), 0) / analytics.length : 0
    },
    dateRange: {
      startDate: rangeStart,
      endDate: rangeEnd
...
const buildFeedbackDataset = async (rangeStart, rangeEnd) => {
  ...
  const [statistics, nps, trendingTopics, recentFeedback] = await Promise.all([feedbackManager.getFeedbackStatistics(filters), feedbackManager.getNPS(filters), feedbackManager.getTrendingTopics(5), Feedback.find({
    createdAt: {
      $gte: rangeStart,
      $lte: rangeEnd
    }
  }).populate("customer", "name email phone").populate("order", "orderNumber").sort({
    createdAt: -1
  }).limit(10).lean()]);
  return {
    statistics,
...
const buildWaiterDataset = async (rangeStart, rangeEnd) => {
  ...
  const [byStatus, summary] = await Promise.all([WaiterCall.aggregate([{
    $match: {
      createdAt
    }
  }, {
...
const buildTableDataset = async () => {
  const [activeTables, available, occupied, reserved] = await Promise.all([Table.countDocuments({
    isActive: true
  ...
  return {
    ...
    occupied,
    reserved,
    occupancyRate: activeTables > 0 ? (occupied + reserved) / activeTables * 100 : 0
  };
};
const buildMenuDataset = async () => {
  const [totalItems, availableItems, unavailableItems, vegetarian, nonVegetarian, vegan, glutenFree] = await Promise.all([MenuItem.countDocuments({
    isActive: true
...
const buildFinanceDataset = async (rangeStart, rangeEnd) => {
  ...
  const [summary, paymentMethods, dailyRevenue, orderTypes, sessionRevenue] = await Promise.all([Order.aggregate([{
    $match: {
      orderPlacedAt,
      paymentStatus: "paid"
    }
...
const buildReportDataset = async (reportType, rangeStart, rangeEnd, tenantId) => {
  ...
  return getOrSetCache(cacheKey, REPORT_DATASET_CACHE_TTL_MS, async () => {
    ...
    const [orders, sessions, kitchen, feedback, menu, tables, waiterCalls] = await Promise.all([buildOrderStatisticsDataset(rangeStart, rangeEnd), buildSessionAnalyticsDataset(rangeStart, rangeEnd), buildKitchenAnalyticsDataset(rangeStart, rangeEnd), buildFeedbackDataset(rangeStart, rangeEnd), buildMenuDataset(), buildTableDataset(), buildWaiterDataset(rangeStart, rangeEnd)]);
    return {
      orders,
      sessions,
      kitchen,
...
exports.generateAnalyticsReportFile = async (req, res) => {
  try {
    ...
    const dataset = await buildReportDataset(normalizedReportType, rangeStart, rangeEnd, req.tenantId || req.user?.tenantId || "default");
    ...
    const result = await generateAnalyticsReport({
      ...
      restaurantName: String(restaurantName || "Restaurant").trim() || "Restaurant",
      generatedAt: new Date(),
      dateRangeLabel: String(dateRangeLabel || "").trim() || `${dateRange.startDate} to ${dateRange.endDate}`,
...
exports.downloadGeneratedReport = async (req, res) => {
  try {
    ...
    if (String(report.tenantId || "") !== String(req.tenantId || req.user?.tenantId || "")) {
