...
exports.getKitchenStations = async (req, res) => {
  try {
    const stations = await KitchenStation.find().select("name stationType capacity colorCode displayOrder status currentLoad preparationTimes").sort({
      displayOrder: 1,
      name: 1
    }).lean();
    const stationsWithCategories = await Promise.all(stations.map(async station => {
...
exports.getKitchenStation = async (req, res) => {
  try {
    const station = await KitchenStation.findById(req.params.id).select("name stationType capacity colorCode displayOrder status currentLoad preparationTimes").lean();
    if (!station) {
...
exports.createKitchenStation = async (req, res) => {
  try {
    ...
    const existingStation = await KitchenStation.findOne({
      name
    });
    if (existingStation) {
    ...
    const shapedStation = shapeKitchenStationResponse(station.toObject ? station.toObject() : station, []);
    res.status(201).json({
      success: true,
...
exports.updateKitchenStation = async (req, res) => {
  try {
    ...
    const updatedStation = await KitchenStation.findByIdAndUpdate(req.params.id, updateData, {
      new: true,
      runValidators: true
    }).lean();
    res.status(200).json({
...
exports.removeCategoryFromStation = async (req, res) => {
  try {
    ...
    if (!category.kitchenStation || category.kitchenStation.toString() !== stationId) {
      return res.status(400).json({
        success: false,
...
exports.getStationDashboard = async (req, res) => {
  try {
    ...
    const menuItems = await require("../models/MenuItem").find({
      category: {
        $in: categoryIds
      }
    }).select("name category preparationTime").populate("category", "name").lean();
    const kitchenOrders = await require("../models/KitchenOrder").find({
    ...
    }).populate("items.station", "name").populate("items.assignedTo", "name").lean();
    const currentOrders = kitchenOrders.map(order => {
      const stationItems = order.items.filter(item => item.station && item.station._id.toString() === station._id.toString());
      return {
        orderId: order._id,
