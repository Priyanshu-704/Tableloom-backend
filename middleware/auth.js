...
exports.checkPermission = (user, permission) => {
  ...
  const effectivePermissions = user.resolvedPermissions || user.getPermissions?.() || [];
...
exports.protect = async (req, res, next) => {
  ...
  try {
    ...
    if (requestTenantId && req.user?.role !== "super_admin" && userTenantId !== requestTenantId) {
...
exports.hasAnyPermission = (...permissions) => {
  return (req, res, next) => {
    ...
    const effectivePermissions = req.user.resolvedPermissions || req.user.getPermissions?.() || [];
    if (!["super_admin", "admin"].includes(String(req.user.role).toLowerCase()) && !normalizedPermissions.some(permission => effectivePermissions.includes(permission))) {
      return res.status(403).json({
        success: false,
        message: "You do not have permission to access this resource"
      });
