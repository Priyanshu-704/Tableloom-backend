...
async function backfillMenuItemPreparationTimes() {
  const result = await MenuItem.updateMany(
    {
      $or: [
        { preparationTime: { $exists: false } },
        { preparationTime: null },
        { preparationTime: { $lt: 1 } },
      ],
    },
    {
  ...
  console.log(
    `[migration] menu items updated: ${result.modifiedCount || 0}`,
  );
}
...
async function backfillOrders() {
  ...
  const customerIds = [
    ...new Set(orders.map((order) => String(order.customer || "")).filter(Boolean)),
  ];
  const menuItemIds = [
    ...new Set(
      orders.flatMap((order) =>
  ...
  const customerSessionById = new Map(
    customers.map((customer) => [String(customer._id), customer.sessionId || ""]),
  );
  ...
  for (const order of orders) {
    const nextSessionId =
      order.sessionId || customerSessionById.get(String(order.customer || "")) || "";
...
async function syncPendingBills() {
  const pendingBills = await Bill.find({
    ...
    .select("_id customerId sessionId subtotal taxAmount serviceCharge discountAmount totalAmount metadata")
    .lean();
  ...
  for (const bill of pendingBills) {
    ...
    if (!customerId && bill.sessionId) {
      const customer = await Customer.findOne({ sessionId: bill.sessionId })
        .select("_id")
        .lean();
