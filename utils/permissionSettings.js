...
const LEGACY_PERMISSION_GROUPS = {
  ...
  menu: [Permissions.CATEGORY_TOGGLE_STATUS, Permissions.MENU_CREATE, Permissions.MENU_EDIT, Permissions.MENU_DELETE, Permissions.MENU_TOGGLE_AVAILABILITY, Permissions.MENU_VIEW_ALL, Permissions.MENU_BULK_OPERATIONS, Permissions.MENU_IMPORT_EXPORT, Permissions.MENU_STATS, Permissions.PRICE_STATS],
  inventory: [Permissions.INVENTORY_VIEW_ALL, Permissions.INVENTORY_CREATE, Permissions.INVENTORY_EDIT, Permissions.INVENTORY_DELETE, Permissions.INVENTORY_ADJUST, Permissions.INVENTORY_STATISTICS],
  orders: [Permissions.ORDER_CREATE, Permissions.ORDER_VIEW_ALL, Permissions.ORDER_VIEW_OWN, Permissions.ORDER_UPDATE, Permissions.ORDER_UPDATE_STATUS, Permissions.ORDER_UPDATE_ITEM_STATUS, Permissions.ORDER_PROCESS_PAYMENT, Permissions.ORDER_DELETE],
  tables: [Permissions.TABLE_CREATE, Permissions.TABLE_EDIT, Permissions.TABLE_DELETE, Permissions.TABLE_VIEW_ALL, Permissions.TABLE_UPDATE_STATUS],
  sessions: [Permissions.SESSION_VIEW_ALL, Permissions.SESSION_UPDATE, Permissions.SESSION_COMPLETE_OFFLINE, Permissions.SESSION_CANCEL, Permissions.SESSION_STATISTICS],
  kitchen: [Permissions.KITCHEN_VIEW_DASHBOARD, Permissions.KITCHEN_ACCEPT_ORDER, Permissions.KITCHEN_START_PREPARING, Permissions.KITCHEN_MARK_READY, Permissions.KITCHEN_MARK_SERVED, Permissions.KITCHEN_MANAGE_STATIONS],
  cart: [Permissions.CART_MANAGE, Permissions.CART_CHECKOUT, Permissions.CART_APPLY_DISCOUNT],
  feedback: [Permissions.FEEDBACK_VIEW_ALL, Permissions.FEEDBACK_RESPOND, Permissions.FEEDBACK_STATISTICS],
  notifications: [Permissions.NOTIFICATION_VIEW, Permissions.NOTIFICATION_ANNOUNCE],
  waiter_calls: [Permissions.WAITER_CALL_ACKNOWLEDGE, Permissions.WAITER_CALL_COMPLETE, Permissions.WAITER_CALL_VIEW_ALL, Permissions.WAITER_CALL_STATISTICS],
  reports: [Permissions.VIEW_REPORTS, Permissions.VIEW_DASHBOARD, Permissions.VIEW_STATISTICS],
  settings: [Permissions.SYSTEM_SETTINGS],
  backup: [Permissions.BACKUP_RESTORE]
};
const normalizePermission = permission => String(permission || "").trim().replace(/[\s-]+/g, "_").toLowerCase();
...
const expandPermissionEntries = (permissions = []) => {
  ...
  permissions.forEach(permission => {
    ...
    const matchedEntry = Object.entries(Permissions).find(([key]) => key.toLowerCase() === normalized);
    if (matchedEntry) {
...
const getSettingsDocument = async () => AppSetting.findOne({
...
const buildRolePermissionsMap = async () => {
  ...
  storedRoles.forEach(roleDefinition => {
    ...
    rolePermissions[roleKey] = expandPermissionEntries(roleDefinition?.permissions || []);
  });
