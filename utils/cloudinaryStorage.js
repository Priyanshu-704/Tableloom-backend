...
const normalizeFolder = (...parts) => parts.filter(Boolean).map(part => String(part).replace(/^\/+|\/+$/g, "")).filter(Boolean).join("/");
const resolveFolder = (folder = "") => normalizeFolder(CLOUDINARY_FOLDER, folder);
