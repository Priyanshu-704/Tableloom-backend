...
const normalizeFolder = (value = "") => String(value || "").replace(/^\/+|\/+$/g, "").replace(/[^a-zA-Z0-9/_-]+/g, "-").replace(/\/{2,}/g, "/");
const sanitizeFilenameBase = (value = "image") => String(value || "image").trim().replace(/\.[^.]+$/, "").replace(/[^a-zA-Z0-9_-]+/g, "-").replace(/-{2,}/g, "-").replace(/^-+|-+$/g, "").toLowerCase() || "image";
const getExtensionForMimeType = (mimetype = "") => String(mimetype).toLowerCase() === "image/png" ? ".png" : ".jpg";
const buildRelativePath = (folder, variant, filename) => path.posix.join("uploads", normalizeFolder(folder), variant, filename);
const buildAbsolutePath = (relativePath = "") => path.join(process.cwd(), String(relativePath || ""));
const buildUniqueFilename = (originalname = "image", suffix = "", mimetype = "") => `${sanitizeFilenameBase(originalname)}-${Date.now()}-${crypto.randomBytes(6).toString("hex")}${suffix}${getExtensionForMimeType(mimetype)}`;
...
const createThumbnailBuffer = async ({
  buffer,
  mimetype
}) => toBuffer(sharp(buffer).rotate().resize(THUMBNAIL_SIZE, THUMBNAIL_SIZE, {
  fit: "cover",
  position: "centre"
}), mimetype);
const createOriginalBuffer = async ({
  buffer,
  mimetype
...
const saveImageVariants = async ({
  ...
  const [originalBuffer, thumbnailBuffer] = await Promise.all([createOriginalBuffer({
    buffer,
    mimetype
  ...
  if (isCloudinaryConfigured) {
    const [imageUpload, thumbnailUpload] = await Promise.all([uploadBuffer({
  ...
  const thumbnailFileName = buildUniqueFilename(originalname, "-thumb", mimetype);
  const [image, thumbnail] = await Promise.all([writeLocalImage({
    buffer: originalBuffer,
...
const deleteImageVariants = async ({
  ...
  const shouldDeleteRemote = provider === "cloudinary" || imagePublicId && !String(imagePublicId).startsWith("uploads/") || thumbnailPublicId && !String(thumbnailPublicId).startsWith("uploads/");
  if (shouldDeleteRemote) {
    await Promise.allSettled([imagePublicId ? deleteAsset(imagePublicId, "image") : null, thumbnailPublicId ? deleteAsset(thumbnailPublicId, "image") : null]);
  }
  await Promise.allSettled([deleteLocalFile(imagePublicId || image), deleteLocalFile(thumbnailPublicId || thumbnail)]);
};
const isRemoteAsset = (value = "") => /^https?:\/\//i.test(String(value || ""));
const isLocalAsset = (value = "") => String(value || "").startsWith("uploads/");
const getStoredAssetReference = (record = {}, {
  variant = "image",
  originalField = "image",
...
const serveStoredAsset = async (res, storedReference, options = {}) => {
  ...
  if (isLocalAsset(storedReference)) {
    ...
    const contentType = options.contentType || (extension === ".png" ? "image/png" : "image/jpeg");
    res.setHeader("Content-Type", contentType);
    res.setHeader("Cache-Control", "public, max-age=86400");
    if (options.disposition) {
      res.setHeader("Content-Disposition", options.disposition);
