...
const { deleteAsset, isCloudinaryConfigured, uploadBuffer } = require("./cloudinaryStorage");
...
const buildUniqueFilename = (originalname = "image", suffix = "", mimetype = "") =>
  `${sanitizeFilenameBase(originalname)}-${Date.now()}-${crypto
    .randomBytes(6)
    .toString("hex")}${suffix}${getExtensionForMimeType(mimetype)}`;
...
const saveImageVariants = async ({
  ...
  const [originalBuffer, thumbnailBuffer] = await Promise.all([
    createOriginalBuffer({ buffer, mimetype }),
    createThumbnailBuffer({ buffer, mimetype }),
  ]);
...
const getStoredAssetReference = (
  record = {},
  { variant = "image", originalField = "image", thumbnailField = "thumbnail" } = {}
) => {
