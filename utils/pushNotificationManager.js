...
const INVALID_FCM_TOKEN_ERRORS = new Set(["messaging/invalid-registration-token", "messaging/registration-token-not-registered"]);
...
const normalizeDataPayload = (data = {}) => Object.entries(data || {}).reduce((accumulator, [key, value]) => {
...
const getPrimaryLink = (actions = [], fallbackLink = "") => {
  ...
  const linkAction = actions.find(action => action?.type === "link" && String(action.action || "").trim());
...
class PushNotificationManager {
  ...
  async unregisterToken({
    ...
    return PushNotificationToken.findOneAndUpdate(query, {
      $set: {
        isActive: false,
        lastUsedAt: new Date()
      }
  ...
  async sendNotificationPush(notification = {}) {
    ...
    if (notification.customerSessionId) {
      tokenRecords = await this.getCustomerSessionTokens(tenantId, notification.customerSessionId);
