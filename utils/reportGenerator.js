...
const REPORT_CLEANUP_INTERVAL_MS = Math.max(
  Number(process.env.REPORT_CLEANUP_INTERVAL_MS || 15 * 60 * 1000),
  60 * 1000,
);
const MAX_REGISTERED_REPORTS = Math.max(
  Number(process.env.MAX_REGISTERED_REPORTS || 200),
  25,
);
...
const buildAnalyticsSummaryCards = (dataset = {}, currency = "INR") => [
  ...
  },
  {
    label: "Orders",
    value: numberValue(
      dataset?.orders?.todayOrders || dataset?.orders?.totalOrders || 0,
    ).toLocaleString(),
  },
  {
    label: "Completed Sessions",
    value: numberValue(
      dataset?.sessions?.completedSessions || 0,
    ).toLocaleString(),
  },
  {
    label: "NPS Score",
    value: Math.round(
      numberValue(
        dataset?.feedback?.nps?.score || dataset?.feedback?.nps?.nps || 0,
      ),
    ).toString(),
  },
];
const buildAnalyticsOrderStatusRows = (dataset = {}) =>
  Object.entries(dataset?.orders?.statusCounts || {}).map(
    ([status, count]) => ({
...
const buildAnalyticsOperationsRows = (dataset = {}) => [
  ...
  },
  {
    ...
    value: formatMinutes(
      dataset?.kitchen?.overallStats?.avgPreparationTime || 0,
    ),
  },
  {
...
const buildAnalyticsWaiterStatusRows = (dataset = {}) =>
  (dataset?.waiterCalls?.byStatus || []).map((row) => ({
    ...
    averageTime: formatMinutes(
      row?.avgResponseTime || row?.avgResolutionTime || 0,
    ),
  }));
...
const buildFinanceBreakdownRows = (dataset = {}, currency = "INR") => [
  ...
  },
  {
    ...
    value: formatCurrency(
      dataset?.summary?.averageSessionRevenue || 0,
      currency,
    ),
  },
  {
    ...
    value: numberValue(
      dataset?.summary?.completedSessions || 0,
    ).toLocaleString(),
  },
];
...
const renderSummaryCards = (doc, cards = [], currentY) => {
  ...
  cards.forEach((card, index) => {
    ...
    doc
      .roundedRect(x, nextY, cardWidth, cardHeight, 14)
      .fillAndStroke("#f8fafc", "#dbeafe");
    doc
      .fillColor("#475569")
      .font(getPdfFontName("regular"))
      .fontSize(10)
      .text(card.label, x + 12, nextY + 12, {
        width: cardWidth - 24,
      });
    doc
      .fillColor("#0f172a")
...
const renderBarChart = (doc, title, rows = [], currentY, options = {}) => {
  ...
  const maxValue = Math.max(
    ...rows.map((row) => numberValue(row.quantity ?? row.value)),
    1,
  );
  rows.forEach((row) => {
    ...
    const width = Math.max(
      18,
      (numericValue / maxValue) * (options.chartWidth || 220),
    );
    doc
      .font(getPdfFontName("regular"))
      .fontSize(10)
    ...
    doc
      .roundedRect(
        ...
        y + 2,
        options.chartWidth || 220,
        10,
        999,
      )
      .fill("#e2e8f0");
    doc
      .roundedRect(doc.page.margins.left + 180, y + 2, width, 10, 999)
      .fill(options.barColor || "#0f766e");
    doc
      .fillColor("#475569")
      .font(getPdfFontName("regular"))
      .fontSize(10)
...
const renderPieChart = (doc, title, rows = [], currentY, options = {}) => {
  ...
  doc
    .circle(centerX, centerY, radius * 0.58)
...
const renderTwoColumnTable = (doc, title, rows = [], currentY) => {
  ...
  if (!rows.length) {
    doc
      ...
      .fontSize(10)
      .fillColor("#64748b")
      .text("No data available", tableX, y);
    return y + 20;
  }
  rows.forEach((row, index) => {
    ...
    doc
      ...
      .fontSize(10)
      .fillColor("#111827")
      .text(row.metric || row.status || row.label, tableX + 12, y);
  ...
  const bodyMarkup = chartRows
    .map((row, index) => {
      ...
      const barWidth = Math.max(
        12,
        Math.round((numericValue / maxValue) * trackWidth),
      );
      const y = chartTop + index * rowHeight;
      const valueLabel = row.secondary
        ? `${row.value} | ${row.secondary}`
        : row.value;
      return `
...
const buildChartPngBuffer = async (chart = {}) => {
  const svg =
    chart.type === "pie" ? buildPieChartSvg(chart) : buildBarChartSvg(chart);
  return sharp(Buffer.from(svg)).png().toBuffer();
};
const styleMergedRange = (
  worksheet,
  startRow,
  endRow,
  startCol,
...
const addTableToWorksheet = (worksheet, section = {}, startRow = 1) => {
  ...
  section.rows.forEach((item, index) => {
    ...
    worksheet.getCell(`A${row}`).value =
      item.metric || item.status || item.label || "-";
    worksheet.getCell(`E${row}`).value =
      item.value || item.count || item.averageTime || "-";
    styleMergedRange(worksheet, row, row, 1, 8, {
      fill:
        index % 2 === 0
          ? {
...
const buildExcelWorkbookBuffer = async ({
  ...
  let currentRow = addSummaryCardsToWorksheet(
    worksheet,
    sections.summaryCards,
    6,
  );
  for (const chart of sections.charts) {
    currentRow = await addChartToWorksheet(
      workbook,
      worksheet,
      chart,
      currentRow,
...
const buildExcelMarkup = ({
  ...
  const firstTableTitle = isFinance
    ? "Payment Method Revenue"
    : "Order Status Summary";
  const secondTableTitle = isFinance
    ? "Order Type Revenue"
    : "Operations Summary";
  const thirdTableTitle = isFinance
    ? "Revenue Breakdown"
    : "Waiter Call Status";
  return `
            ...
            : chartRows.length
              ? chartRows
                  .map((row) => {
                    const width = Math.max(
                      8,
                      Math.round((row.quantity / maxChartValue) * 100),
                    );
                    return `
                    <div class="chart-row">
...
const buildPdfBuffer = ({
  ...
  new Promise((resolve, reject) => {
    ...
    doc.on("error", reject);
    doc
      .roundedRect(40, 40, doc.page.width - 80, 94, 18)
      .fillAndStroke("#eff6ff", "#dbeafe");
    doc
      .fillColor("#0f172a")
      .font(getPdfFontName("bold"))
      .fontSize(22)
      .text(reportTitle, 56, 58);
...
const generateAnalyticsReport = async ({
  ...
  const filePath = path.join(
    REPORT_OUTPUT_DIR,
    `${sanitizeFileSegment(tenantId || "tenant")}-${Date.now()}-${crypto.randomBytes(6).toString("hex")}.${extension}`,
  );
  if (format === "excel") {
