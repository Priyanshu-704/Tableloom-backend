...
module.exports = (contextPath = "") => {
  const normalizedContextPath = contextPath && contextPath !== "/" ? `/${String(contextPath).replace(/^\/+|\/+$/g, "")}` : "";
  const configuredBackendUrl = String(process.env.BACKEND_URL).replace(/\/+$/, "");
  const serverUrl = configuredBackendUrl.endsWith(normalizedContextPath) ? configuredBackendUrl : `${configuredBackendUrl}${normalizedContextPath}`;
  return swaggerJsdoc({
    definition: {
      openapi: "3.0.0",
      info: {
